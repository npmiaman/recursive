...
import { applyFix } from "../agents/fix.ts";
...
export async function hillClimb(issue: Issue, options: RunOptions = {}): Promise<RunResult> {
  ...
  try {
    ...
    log(formatScore(baseline).split("\n").map((l) => "  " + l).join("\n"));

    journal(issue.id, { type: "baseline", issue, score: baseline, at: new Date().toISOString() });

    if (baseline.total <= GOOD_ENOUGH) {
      log(`  baseline already clean (${baseline.total.toFixed(4)}) — the probe cannot reproduce this.`);
      ...
      return {
        issue,
        investigation: { hypothesis: "Not reproducible in the probe environment.", suspectSelectors: [], searchTerms: [], needsExternalResearch: false, directions: [] },
        baseline,
        final: baseline,
        improvement: 0,
        iterations: [],
    ...
    journal(issue.id, { type: "investigation", investigation, researchNotes, at: new Date().toISOString() });
    ...
    for (let n = 1; n <= maxIterations; n++) {
      ...
      try {
        const attempt = await applyFix(issue, direction, investigation, researchNotes, n, tried);
        ...
        if (delta < -MIN_IMPROVEMENT) {
          ...
          log(`      ✓ KEPT   ${entry.scoreBefore.toFixed(4)} → ${after.total.toFixed(4)}  (${delta.toFixed(4)})`);
        } else {
          git.revertTo(checkpoint);
          entry.outcome = "reverted";
          entry.note = attempt.summary.slice(0, 600);
          log(`      ✗ revert ${entry.scoreBefore.toFixed(4)} → ${after.total.toFixed(4)}  (${delta >= 0 ? "+" : ""}${delta.toFixed(4)})`);
        }
    ...
    journal(issue.id, { type: "summary", improvement, kept: acceptedCommits.length, tried: iterations.length, at: new Date().toISOString() });
    return result;
  } finally {
    await scorer.close();
  }
